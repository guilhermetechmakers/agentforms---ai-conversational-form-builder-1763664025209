import { FileText, Calendar, Hash, Download } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { useLegalDocument, useDownloadDPA } from '@/hooks/useLegal';
import { format } from 'date-fns';
import { Loader2 } from 'lucide-react';
import { cn } from '@/lib/utils';

export function DataProcessingAddendumPage() {
  const { data: document, isLoading, error } = useLegalDocument('data-processing-addendum');
  const { mutate: downloadDPA, isPending: isDownloading } = useDownloadDPA();
        ...
        <Card className="max-w-2xl">
          <CardContent className="pt-6 text-center">
            <p className="text-medium-gray">
              Unable to load the Data Processing Addendum. Please try again later.
            </p>
          </CardContent>
        </Card>
      </div>
    ...
    <div className="min-h-screen bg-background py-8 md:py-12">
      <div className="max-w-4xl mx-auto px-4 md:px-6 lg:px-8">
        ...
        <Card className="animate-fade-in-up animation-delay-100">
          <CardHeader>
            <CardTitle className="text-2xl">Data Processing Addendum</CardTitle>
          </CardHeader>
          <CardContent>
            <div
              className={cn(
